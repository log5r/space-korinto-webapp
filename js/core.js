...
const timedGateGrace = clock => clock >= 10 && clock % 10 < 1;
function holeReward(h, modeId, side, clock = 0) {
  const lit = modeId === 'infinite' || h.type === 'black' || (modeId === 'timed' && timedGateGrace(clock)) || Math.sign(h.x - CX) === side;
