...
window.SpaceKorinto.createBoard = function (timed) {
'use strict';
timed = !!timed;
...
const laneX = [], lanes = [];
if (timed) {
  // Clear chutes separate launch powers before asteroid pins scatter the ball.
  // Keep 36 units below the arc so a ball can ride over each guide tip.
  for (const x of [160, 300, 440]) lanes.push({ x, y: 240, r: 9, lit: false, halfWidth: 62 });
  for (const l of lanes) {
    for (const x of [l.x - 70, l.x + 70]) {
      laneX.push(x);
      seg(x, Math.max(128, ARC_CY - Math.sqrt(ARC_R ** 2 - (x - ARC_CX) ** 2) + 36), x, 263, { w: 5, e: 0.35 });
    }
    box(l.x - 88, l.y - 65, 176, 110);
  }
} else {
  laneX.push(250, 283, 317, 350);
  for (const x of laneX) seg(x, 128, x, 178, { w: 5, e: 0.35 });
  lanes.push(...[266, 300, 334].map(x => ({ x, y: 160, r: 9, lit: false })));
  box(222, 104, 156, 100);
}
...
return { timed, gate, pins, segs, laneX, lanes, bumpers, holes, stars, slings, flippers, drain, pinsNear, mirror };
};
